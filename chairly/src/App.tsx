...
function App() {
  ...
  const [staff, setStaff] = useState<Staff[]>(() => JSON.parse(localStorage.getItem('chairly-staff') || 'null') || INITIAL_STAFF)
  const [bookings, setBookings] = useState<Booking[]>(() => JSON.parse(localStorage.getItem('chairly-bookings') || 'null') || seedBookings)
...
function ShopHeader() {
  return <section className="shop-header">
    ...
    <div className="shop-copy"><span className="eyebrow">Brighton · Barbershop</span><h1>North Laine Barber Co.</h1><p><MapPin size={16}/>18 Gardner Street, Brighton</p><div className="rating"><strong>4.9</strong><span>★★★★★</span><small>127 reviews</small></div></div>
  </section>
}
...
function BookingFlow({ staff, bookings, editing, onSave, onExit }: { staff: Staff[]; bookings: Booking[]; editing?: Booking; onSave: (b: Booking) => void; onExit: () => void }) {
  ...
  const submit = () => {
    ...
    onSave({ id: editing?.id || crypto.randomUUID(), customer: customer.trim(), email: email.trim(), serviceId, staffId, date, time, status: 'confirmed' })
  }
  ...
  return <>
    ...
    <section className="booking-layout">
      <div className="booking-card">
        ...
        </div><button className="primary" disabled={!serviceId} onClick={() => setStep(1)}>Continue</button></div>}
        ...
        </div><button className="primary" disabled={!staffId} onClick={() => { if (staffId === 'any') setStaffId(staff[0].id); setTime(''); setStep(2) }}>Continue</button></div>}
        ...
        {step === 2 && <div className="panel"><Back onClick={() => setStep(1)}/><span className="eyebrow">Step 3 of 4</span><h2>Choose a time</h2><p className="muted">Times shown are local to Brighton.</p><div className="date-strip">{dates.map(item => <button className={date === item ? 'selected' : ''} key={item} onClick={() => { setDate(item); setTime('') }}><small>{formatDate(item, true).split(' ')[0]}</small><strong>{new Date(`${item}T12:00:00`).getDate()}</strong><span>{new Intl.DateTimeFormat('en-GB', { month: 'short' }).format(new Date(`${item}T12:00:00`))}</span></button>)}</div>
          <h3>{formatDate(date)}</h3>{slots.length ? <div className="slots">{slots.map(slot => <button className={time === slot ? 'selected' : ''} onClick={() => setTime(slot)} key={slot}>{slot}</button>)}</div> : <div className="empty"><CalendarDays size={22}/><strong>No times available</strong><span>Choose another date or professional.</span></div>}<button className="primary" disabled={!time} onClick={() => setStep(3)}>Continue</button>
        </div>}

        {step === 3 && <div className="panel"><Back onClick={() => setStep(2)}/><span className="eyebrow">Step 4 of 4</span><h2>Your details</h2><p className="muted">We’ll use these details to identify your booking.</p><label>Full name<input value={customer} onChange={e => setCustomer(e.target.value)} placeholder="e.g. James Taylor" autoComplete="name"/></label><label>Email address<input value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" type="email" autoComplete="email"/></label><div className="summary"><div><Scissors size={17}/><span><small>Service</small><strong>{service?.name}</strong></span></div><div><UserRound size={17}/><span><small>With</small><strong>{person?.name}</strong></span></div><div><CalendarDays size={17}/><span><small>When</small><strong>{formatDate(date, true)} at {time}</strong></span></div></div><button className="primary" disabled={!customer.trim() || !email.includes('@')} onClick={submit}>{editing ? 'Save changes' : 'Confirm booking'}</button><p className="fine-print">No payment is required. You can change or cancel this booking later.</p></div>}
      </div>
